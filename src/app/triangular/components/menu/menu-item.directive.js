(function() {
    ...
    /* @ngInject */
    function triMenuItemController($scope, $rootScope, $injector, $mdSidenav, $state, $filter, $window) {
        var triMenuItem = this;

        triMenuItem.clickMenu = clickMenu;
        ...
        function clickMenu(nameMenu) {
            $state.go('triangular.notes')
                .then(function() {
                    if (nameMenu === 'Notes') {
                        $rootScope.$broadcast('clickNotes');
                    }
                    else {
                        $rootScope.$broadcast('clickLabel', {data: nameMenu});
                    }
                });
        }
    }
